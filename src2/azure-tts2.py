...
class azure_tts(utils):
    def __init__(self) -> None:
        super().__init__()
        self.tmp_dir=self.path_join('tmp','20230211') # path to tmp dir 
...
#            input('wait!')
    ...
    def combine_audio(self,audio_fps : list,out_fp):
        ...
        return 
    # changes speed of a .wav file with ffmpeg 
    ...
    def add_pause(self,vid_fp,duration=0.0,swap = False ):
...
#gtts#fp=self.gtts(out_filename=hash,txt=txt,ssms=True,rate=1.0)
            ...
            fp=self.tts(out_filename=hash,txt=txt,ssms=True,rate=1.25)      # wtf azure throttling like 1 request every month 
...
#gtts#self.change_speed(vid_fp=fp,ratio=ratio,swap=True)
            fp=self.tts(out_filename=hash,txt=txt,ssms=True,rate=ratio)
            self.add_pause(vid_fp=fp,swap=True)
